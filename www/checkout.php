...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_POST['accion_checkout']) && $_POST['accion_checkout'] === 'confirmar_pedido' && $direccion) {
            ...
            if (empty($error)) {
                ...
                $stmtPed = $pdo->prepare("INSERT INTO PEDIDO (id_user, total, estado, fecha_pedido) VALUES (:id_user, :total, 'Procesado', NOW())");
                $stmtPed->execute([
                    'id_user' => $id_user,
                    'total'   => $total_pedido
                ]);
    ...
    // Recálculo del resumen para la interfaz
    $subtotal_general = 0;
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        //Guardar dirección si el usuario no tenía una registrada
        if (isset($_POST['accion_checkout']) && $_POST['accion_checkout'] === 'guardar_direccion') {
            $calle  = sanear_string($_POST['calle'] ?? '');
            $ciudad = sanear_string($_POST['ciudad'] ?? '');
            $cp     = sanear_string($_POST['codigo_postal'] ?? '');
        ...
        //Confirmar compra e iniciar Transacción SQL
        if (isset($_POST['accion_checkout']) && $_POST['accion_checkout'] === 'confirmar_pedido' && $direccion) {
